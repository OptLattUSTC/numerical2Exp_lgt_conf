function stat_n = pxpM_site_fill_more_statFcn(psic,basis,gauge_T,matter_T)
...
%%
gauge_sub = gauge_MtA(basis.GL_idx_lt,:);
% matter_sub = matter_MtA(basis.GL_idx_lt,:);
prob_gl_sub = prob_ltC_GL(basis.GL_idx_lt);
sum(prob_gl_sub)

matter_T0 = [basis.matter_init(1),matter_T,...
    basis.matter_init(end)];

% matter_sub0 = [repmat(matter_T0(1),size(matter_sub,1),1),...
%     matter_sub,repmat(matter_T0(end),size(matter_sub,1),1)];

matter_sub0 = basis.matter_All(basis.GL_idx_lt,:);
...
gauge_DensitylC = sum(gauge_MtA .* repmat(prob_ltC_GL,1,L),1,'omitnan');
