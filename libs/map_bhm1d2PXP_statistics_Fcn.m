function stat_n = map_bhm1d2PXP_statistics_Fcn(psic,basis,gauge_T,matter_T)
...
%%
matter_occupation_lC = sum(matter_MtA .* repmat(prob_ltC_GL,1,L),1,'omitnan');
gauge_occupation_lC = sum(gauge_MtA .* repmat(prob_ltC_GL,1,L-1),1,'omitnan');

% old case
gauge_DensitylC = sum(abs(sum(gauge_MtA,2) .* prob_ltC_GL),'omitnan')/(L-1);
gauge_abs_orderP_Cur = sum(abs(sum(gauge_MtA,2) .* prob_ltC_GL),'omitnan')/(L-1);
% 
gauge_orderP_Cur = sum(sum(gauge_MtA,2) .* prob_ltC_GL,'omitnan')/(L-1);
...
for kk = 1:L
    ...
    G_dis_r(kk) = sum(G_cur .* prob_gl_sub,1,'omitnan')/(L-r);
