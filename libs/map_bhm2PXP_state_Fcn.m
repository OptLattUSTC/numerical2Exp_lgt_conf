function [basis] = map_bhm2PXP_state_Fcn(st_up,st_dn,basis,st_orig)
%
%
%  ---------------------------------
...
gauge_init = zeros(1,L-1);

for kk = 1:L-1
    if sum(st_pxp_init(kk:kk+1) == [2,0]) == 2
        sz_cur = 0.5;
    else
        sz_cur = -0.5;
    end
    gauge_init(kk) = (-1)^(kk-1)*sz_cur;
end

% initial matter field, the start and end matter sites are defined as zero
% at beginning
matter_init = zeros(1,L);
...
gauge_MtA = nan(n_bs,L-1);

for kk = 1:L-1
    sub_MtC = stateA(:,kk:kk+1);
    idx_C1 = sum(sub_MtC == [2,0],2) == 2;
    gauge_MtA(idx_C1,kk) = (-1)^(kk-1)*1/2;
    
    idx_C2 = (sum(sub_MtC == [1,1],2) == 2) ...
        | (sum(sub_MtC == [1,2],2) == 2) ...
        | (sum(sub_MtC == [0,2],2) == 2) ...
        | (sum(sub_MtC == [0,1],2) == 2);
    gauge_MtA(idx_C2,kk) = -(-1)^(kk-1)*1/2;
    
end

% idx_nonnan = sum(~isnan(gauge_MtA),2) == L-1;
...
%% original state
st_pxp_orig = st_orig;
gauge_st_pxp_orig = zeros(1,L-1);

for kk = 1:L-1
    if sum(st_pxp_orig(kk:kk+1) == [2,0]) == 2
        sz_cur = 0.5;
    else
        sz_cur = -0.5;
    end
    gauge_st_pxp_orig(kk) = (-1)^(kk-1)*sz_cur;
end

gauge_st_pxp_origA = [gauge_edge(1),gauge_st_pxp_orig,gauge_edge(2)];

matter_st_pxp_orig = gauge_st_pxp_origA(2:end) - gauge_st_pxp_origA(1:end-1);
matter_st_pxp_origA = matter_st_pxp_orig;
